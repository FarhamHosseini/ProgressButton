package com.apachat.progressbutton.core;
...
public class ProgressButton extends AppCompatButton implements View.OnTouchListener {
  ...
  private void init() {
    ...
    mButtonColor = resources.getColor(R.color.pb_button_default_color);
    mShadowColor = resources.getColor(R.color.pb_button_default_shadow_color);
    mShadowHeight = resources.getDimensionPixelSize(R.dimen.pb_button_default_shadow_height);
    mCornerRadius = resources.getDimensionPixelSize(R.dimen.pb_button_default_conner_radius);
  ...
  private void parseAttrs(Context context, AttributeSet attrs) {
    ...
    for (int i = 0; i < typedArray.getIndexCount(); i++) {
      ...
      if (attr == R.styleable.ProgressButton_pb_isShadowEnable) {
        isShadowEnabled = typedArray.getBoolean(attr, true); //Default is true
      } else if (attr == R.styleable.ProgressButton_pb_buttonColor) {
        mButtonColor = typedArray.getColor(attr, getResources().getColor(R.color.unpressed_color));
      } else if (attr == R.styleable.ProgressButton_pb_loaderColor) {
        mloaderColor = typedArray.getColor(attr, getResources().getColor(R.color.white));
      } else if (attr == R.styleable.ProgressButton_pb_shadowColor) {
        mShadowColor = typedArray.getColor(attr, getResources().getColor(R.color.pressed_color));
        isShadowColorDefined = true;
      } else if (attr == R.styleable.ProgressButton_pb_shadowHeight) {
        mShadowHeight = typedArray.getDimensionPixelSize(attr, R.dimen.pb_button_default_shadow_height);
      } else if (attr == R.styleable.ProgressButton_pb_cornerRadius) {
        mCornerRadius = typedArray.getDimensionPixelSize(attr, R.dimen.pb_button_default_conner_radius);
      } else if (attr == R.styleable.ProgressButton_pb_isCircular) {
        isCircularButton = typedArray.getBoolean(attr, false);
      } else if (attr == R.styleable.ProgressButton_pb_isLoading) {
        isLoading = typedArray.getBoolean(attr, false);
      } else if (attr == R.styleable.ProgressButton_pb_loaderMargin) {
        mPaddingProgress = mCornerRadius = typedArray.getDimensionPixelSize(attr, R.dimen.pb_button_default_progress_margin);
      } else if (attr == R.styleable.ProgressButton_pb_loaderWidth) {
        mStrokeWidth = mCornerRadius = typedArray.getDimensionPixelSize(attr, R.dimen.pb_button_default_progress_width);
      }
    }
    typedArray.recycle();
